...
function BWWC_XSI_cron_job_worker ($hardcron=false)
{
  ...
  /*
  if (@$bwwc_xsi_settings['gateway_settings']['service_provider'] != 'electrum-xsi-wallet')
  {
    /*
	print "Only active electrum wallet as a service provider needs cron job";
	print "<pre>";
	print_r ($bwwc_xsi_settings);
	print "</pre>";
  ...
  */
  ...
  $confirmations_required = $bwwc_xsi_settings['gateway_settings']['confirmations'];
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $address_meta    = BWWC_XSI_unserialize_address_meta (@$row_for_balance_check['address_meta']);
		  ...
		  $balance_info_array = BWWC_XSI__getreceivedbyaddress_info ($row_for_balance_check['xsi_address'], $confirmations_required, $bwwc_xsi_settings['blockchain_api_timeout_secs']);
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          BWWC_XSI__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['xsi_address']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");
		  //print "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['xsi_address']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.";

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            BWWC_XSI__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['xsi_address']}' (XSI '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
        else
        {
        ...
        //	Scan '$address_meta['orders']' for first UNPAID order that is exactly matching amount at address.
	        ...
	        BWWC_XSI__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['xsi_address']}' (XSI '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        BWWC_XSI__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);
          ...
          //
	        $query =
	          "UPDATE `$xsi_addresses_table_name`
	            ...
	            WHERE `id`='$row_id';";
	        $ret_code = $wpdb->query ($query);
	        BWWC_XSI__log_event (__FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed.");
...
// 	        if (!$hardcron && $ran_cycles >= $bwwc_xsi_settings['soft_cron_max_loops_per_run'])
// 	        {
...
// 	        }
		  ...
		  {
		    BWWC_XSI__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['xsi_address']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
  ...
  //-----------------------------------------------------
  // Pre-generate new stabilityshares address for electrum wallet
  ...
  if ($hardcron)
  {
    ...
    if ($electrum_mpk && @$bwwc_xsi_settings['gateway_settings']['service_provider'] == 'electrum-xsi-wallet')
    {
      ...
      $query =
        "SELECT COUNT(*) as `total_unused_addresses` FROM `$xsi_addresses_table_name`
      ...
      if ($total_unused_addresses < $bwwc_xsi_settings['max_unused_addresses_buffer'])
      {
        BWWC_XSI__generate_new_bitcoin_address_for_electrum_wallet ($bwwc_xsi_settings, $electrum_mpk);
      }
    }
  }
  //-----------------------------------------------------
